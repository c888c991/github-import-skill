---
name: github-import
description: 把本地写好的代码导入 GitHub 仓库的标准流程与记录。包含首次环境配置（gh 登录、提交身份、免密推送）、要导入哪些文件的清单、逐步操作教程、常见证书与网络报错排查，以及每次导入的时间与数据记录。触发词：导入 GitHub、传到 GitHub、推代码上去、上传代码到仓库、新建仓库、git push 失败、查导入记录。
agent_created: true
---
...
- 本地（包括在 Codex 里）写好代码，要传到 GitHub
- 需要新建一个远程仓库
- `git push` 报证书、权限或网络错误
- 要查"上次导入的是什么、什么时候导的"
...
- 本技能有两份：仓库里一份（`skills/github-import/SKILL.md`），本机一份
  （`C:\Users\19106\.workbuddy\skills\github-import\SKILL.md`）。改了任意一份，
  记得同步另一份，否则两边会不一致。
- 在 Codex 里使用时，若它不读 WorkBuddy 的技能目录，可把本文件复制到 Codex 对应的技能目录下。
